...
class ExpFit
{
	...
	void fit(void)
	{
		...
		ret=dlevmar_der(expfunc, jacexpfunc, p.data(), x.data(), m, n, 1000, opts.data(), info.data(), NULL, NULL, this); // with analytic Jacobian
	...
	static void expfunc(double *p, double *x, int m, int n, void *data)
	{
		register int i;
		
		// To access class data and methods
		ExpFit *pClass = (ExpFit *)data;
		std::cout << pClass->p[0] << ", " << pClass->p[1] << ", " << pClass->p[2] << std::endl; 
	...
	// Jacobian of expfunc() 
	static void jacexpfunc(double *p, double *jac, int m, int n, void *data)
	{   
		register int i, j;
	...
	int n, m;
	std::vector<double> p, x, opts, info;
};
